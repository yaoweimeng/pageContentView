...
@interface YWMPageContentView ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation YWMPageContentView
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TYTContentViewWillBeginDragging:)]) {
        [self.delegate TYTContentViewWillBeginDragging:self];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TYTContentViewDidScroll:startIndex:endIndex:progress:)]) {
        [self.delegate TYTContentViewDidScroll:self startIndex:startIndex endIndex:endIndex progress:progress];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(TYTContenViewDidEndDecelerating:startIndex:endIndex:)]) {
        [self.delegate TYTContenViewDidEndDecelerating:self startIndex:startIndex endIndex:endIndex];
    }
}
...
{
    if (!decelerate) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(TYTContenViewDidEndDragging:)]) {
            [self.delegate TYTContenViewDidEndDragging:self];
        }
    }
}
